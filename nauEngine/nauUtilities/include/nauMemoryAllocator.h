/*||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||*/
/**
 * @file nauMemoryAllocator.h
 * @author Marco "Swampy" Millan
 * @date 2019/04/05 2019
 * @brief Nautilus Engine Memory Allocator
 * 
 */
/*||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||°°°||*/
#pragma once
...
#if NAU_PLATFORM == NAU_PLATFORM_LINUX
# include <malloc.h>
#endif
...
namespace nauEngineSDK {
...
#elif NAU_PLATFORM == NAU_PLATFORM_LINUX || NAU_PLATFORM == NAU_PLATFORM_ANDROID
  inline void*
    platformAlignedAlloc16(SIZE_T size) {
  ...
  {
  public:
    static NAU_UTILITY_EXPORT uint64
      getNumAllocs() {
      return m_allocs;
    }
  ...
  private:
     ...
     */
    static NAU_UTILITY_EXPORT void
      incrementAllocCount() {
      ++m_allocs;
    }
    ...
    static NAU_THREADLOCAL uint64 m_allocs;
    static NAU_THREADLOCAL uint64 m_frees;
  };
  ...
  T*
    nau_new(Args&& ...args) {
    return new (nau_alloc<T, Alloc>()) T(forward<Args>(args)...);
  }
  ...
  template<class T, class Alloc = GenAlloc>
  inline void
    nau_deleteN(T* ptr, SIZE_T count) {
    for (SIZE_T i = 0; i < count; ++i) {
      ptr[i].~T();
    }
    MemoryAllocator<Alloc>::free(ptr);
   ...
   */
  inline void*
    nau_alloc_aligned(SIZE_T count, SIZE_T align) {
   ...
   */
  inline void*
    nau_alloc_aligned16(SIZE_T count) {
  ...
  */
  ...
  T*
    nau_new(Args&& ...args) {
    return new (nau_alloc<T, GenAlloc>()) T(forward<Args>(args)...);
  }
   ...
   */
   /***************************************************************************/
#define NAU_PVT_NEW(T, ...)                                                    \
      new (nau_alloc<T, GenAlloc>()) T(__VA_ARGS__)

#define NAU_PVT_DELETE(T, ptr)                                                 \
      (ptr)->~T();                                                            \
      MemoryAllocator<GenAlloc>::free(ptr);

#define NAU_PVT_DELETE_A(T, ptr, Alloc)                                        \
      (ptr)->~T();                                                            \
      MemoryAllocator<Alloc>::free(ptr);
  ...
  public:
    ...
    static T*
      ...
      void* const pv = nau_alloc<Alloc>(num * sizeof(T));
      if (!pv) {
        return nullptr;
      }
